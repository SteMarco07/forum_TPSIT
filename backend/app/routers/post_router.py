...
@router.get("/topic/{topic_id}/full", response_model=list[PostFullResponse])
def list_posts_full_by_topic(topic_id: int, db: Session = Depends(get_db)):
    return get_post_full_by_topic(topic_id, db)
